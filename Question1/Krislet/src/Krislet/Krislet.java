...
class Krislet implements SendCommand
{
	...
	String	team = "University";
        String strAgentFunction = "AgentFunction.txt";
	try
	    {
		...
		for( int c = 0 ; c < a.length ; c += 2 )
			...
			if( a[c].compareTo("-host") == 0 )
				...
				team = a[c+1];
                        ...
                        //else if( a[c].compareTo("-AgentFunction") == 0)
                        //{
                        //    strAgentFunction = a[c+1];
                        //}
			else
			    {
				throw new Exception();
        ...
        if(!(strAgentFunction.contains("\\") || strAgentFunction.contains("/")))
        {
            strAgentFunction = Paths.get(strAgentFunction).toAbsolutePath().toString();
        }
	Krislet player = new Krislet(InetAddress.getByName(hostName),
				     port, team, strAgentFunction);
    ...
    public Krislet(InetAddress host, int port, String team, String strAgentFunction) 
    ...
    public String m_strAgentFunction;
    //===========================================================================
    // Private members
    // class members
    ...
    // constants
    private static final int	MSG_SIZE = 8192;	// Size of socket buffer
